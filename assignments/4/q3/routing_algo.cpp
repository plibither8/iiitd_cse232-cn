...
void RoutingNode::recvMsg(RouteMsg *msg) {
  ...
  for (auto &their_entry : msg->mytbl->tbl) {
    ...
    int their_cost =
        their_entry.share_cost > 0 ? their_entry.share_cost : their_entry.cost;
    ...
    for (auto &my_entry : mytbl.tbl) {
      if (their_entry.dstip == my_entry.dstip) {
        ...
        if (my_entry.nexthop == msg->from)
          my_entry.cost = min(their_cost + 1, 16);
        else if (their_cost + 1 < my_entry.cost) {
    ...
    if (!is_common_entry) {
      ...
      new_entry.cost =
          msg->from == their_entry.dstip ? 1 : min(their_cost + 1, 16);
