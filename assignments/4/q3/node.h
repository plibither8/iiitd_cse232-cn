...
/*
  ...
  ip_interface: nexthop is reachable via this interface (a node can have
  multiple interfaces) cost: 	cost of reaching dstip (number of hops)
*/
class RoutingEntry {
public:
...
class RouteMsg {
public:
  string from; // I am sending this message, so it must be me i.e. if A is
               // sending mesg to B then it is A's ip address
...
/*
  ...
  ip: 		Own ip
  connectedTo: 	An address to which above mentioned ip is connected via
  ethernet.
*/
class NetInterface {
private:
...
/*
  ...
  Node* is part of each interface, it easily allows to send message to another
  node mytbl: 		Node's routing table
*/
class Node {
private:
  string name;
...
protected:
  ...
  virtual void recvMsg(RouteMsg *msg) { cout << "Base" << endl; }
...
public:
  ...
  void sendMsg() {
    ...
    for (int i = 0; i < mytbl.tbl.size(); ++i) {
      ...
      for (int i = 0; i < interfaces.size(); ++i) {
        if (interfaces[i].first.getConnectedIp() == entry.nexthop) {
          entry.share_cost = 16;
          break;
        }
      }
